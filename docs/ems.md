...
### 一、**FS的定义**
FS是**Full Scale（满量程）**的缩写，表示测量仪器或传感器的最大可测范围。例如：
- 某温度传感器量程为0~500℃，其FS即为500℃；
- 电压表量程为0~1000V，FS则为1000V。

---

### 二、**测量精度中的FS计算**
精度表示为`±x% FS`时，实际误差为满量程的百分比值，计算公式为：
**误差值 = 测量量程 × x%**
例如：
一台压力传感器量程为0~200kPa，精度为0.5% FS，则其最大允许误差为 `200kPa × 0.5% = ±1kPa`。

---

### 三、**注意事项**
1. **FS与实际量程的差异**
   - 部分仪器显示的FS可能与实际量程不同。例如：电压表量程为600V，但最大显示值为2000V，此时FS按2000V计算；
2. **与其他精度表示法的区别**
   - **RD（读数精度）**：误差基于当前测量值（如±0.1% RD）；
   - **FS（满量程）**：误差基于最大量程（如±0.2% FS）。

---

### 四、**应用场景示例**
- **工业传感器**：流量计标注精度为±0.3% FS，若其FS为1000L/min，则误差范围为±3L/min；
- **实验室仪器**：电子秤量程0~10kg，精度0.05% FS，误差为±5g。

---

### 总结
FS是衡量仪器精度的关键参数，需结合量程范围理解其实际误差范围，尤其在对比不同设备精度时，需区分FS与RD的基准差异。
